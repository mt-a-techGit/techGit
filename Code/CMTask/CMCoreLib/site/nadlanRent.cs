...
namespace CMCore.site
{
    class nadlanRent : baseSite
    {
        ...
        {
            return "http://www.ad.co.il/nadlanrent?df=3&pageindex="+pageNum;
        }

     
        public static string getBasePageUrl()
        {
            return "http://www.ad.co.il/nadlanrent?df=3";
        }
        ...
        private TTaskStatusType getBasisTable()
        {
            try
            {
                System.Collections.ObjectModel.ReadOnlyCollection<IWebElement> lines = myDriver.WebDriver.FindElements(By.ClassName("col-sm-6"));
                if (lines.Count == 0)
